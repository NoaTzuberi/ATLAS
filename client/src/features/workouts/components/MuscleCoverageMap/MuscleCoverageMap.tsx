...
/**
 * Anterior (front-view) muscle silhouette.
 *
 * Source: react-body-highlighter (https://github.com/giavinh79/react-body-highlighter,
 * npm package "react-body-highlighter" v2.0.5)
 * License: MIT — Copyright (c) 2020 GV79
 * Used: the anterior-view SVG polygon coordinates only (its `anteriorData`
 * muscle shapes, in their original 0–100 × 0–200 coordinate space), copied
 * directly and remapped from that library's muscle-slug taxonomy onto our
 * own BodyRegion keys. We did not install the package itself — its <Model>
 * component owns its own data/coloring API, which doesn't fit the
 * exercise-count intensity scaling built here, so only the shape data was
 * reused. See docs/THIRD_PARTY_CONTENT.md for the full notice.
 *
 * Real anatomy note: lats/back, glutes and hamstrings are not visible from
 * the front — the source library only draws them on its separate posterior
 * view, which this diagram doesn't have room for. Rather than relabeling an
 * unrelated front-view shape (obliques, hip, knee) to stand in for them, we
 * render those shapes as permanent neutral filler (see NEUTRAL_POLYGONS
 * below) so the silhouette stays visually whole, and simply never highlight
 * them — those three muscle groups are still tracked correctly in the
 * workout summary's "Muscles covered" chips (see WorkoutSummaryCard), just
 * not on this diagram.
 */
const REGION_POLYGONS: Record<BodyRegion, string[]> = {
  chest: [
...
/** Head, neck, obliques, hip and knee shapes — kept as permanent neutral
 * filler so the silhouette reads as a whole body with no missing chunks at
 * the torso sides, hips or knees. They never highlight: obliques/hip/knee
 * are here specifically because they used to stand in for back/glutes/
 * hamstrings and no longer do (see the attribution comment above). */
const NEUTRAL_POLYGONS = [
...
const NEUTRAL_STROKE = 'rgba(255, 255, 255, 0.16)';
...
function RegionShape({ region, count }: RegionShapeProps) {
  const style = {
  ...
  return (
    <g
      className="muscle-map-region"
      aria-label={`${BODY_REGION_LABELS[region]}: ${count} exercise${count === 1 ? '' : 's'}`}
    >
      {REGION_POLYGONS[region].map((points, index) => (
        <polygon key={index} points={points} stroke={NEUTRAL_STROKE} style={style} />
      ))}
    </g>
  );
}
...
export function MuscleCoverageMap({ rows }: MuscleCoverageMapProps) {
  ...
  return (
    <GlassCard className="muscle-map-card" variant="flat">
      <span className="muscle-map-title">Muscle Coverage</span>

      <svg className="muscle-map-figure" viewBox="0 0 100 200" role="img" aria-label="Muscle coverage diagram">
        {NEUTRAL_POLYGONS.map((points, index) => (
          <polygon key={index} points={points} fill={NEUTRAL_FILL} stroke={NEUTRAL_STROKE} />
        ))}

        {(Object.keys(REGION_POLYGONS) as BodyRegion[]).map((region) => (
          <RegionShape key={region} region={region} count={coverage[region]} />
        ))}
      </svg>
